...
class OHLCVWriter:
    ...
    def write(self, candle: OHLCV, overwrite: bool = False) -> None:
        """
        ...
        :param overwrite: Overwrite existing OHLCV data if True
        ...
        if self._size == 0:
            ...
            # Overwrite 모드가 아니고, write 하려는 캔들의 timestamp 가 이미 있는 경우에는 write 하지 않고 return
            if self._last_timestamp is not None and candle.timestamp == self._last_timestamp and not overwrite:
                return
    ...
    def overwrite(self, timestamp: int, candle: OHLCV) -> None:
        if timestamp != candle.timestamp:
            raise ValueError("Timestamp does not match")

        self.seek_to_timestamp(timestamp)
        self.write(candle, overwrite=True)
    ...
    def truncate(self) -> None:
        ...
        # truncate 한 자리에 동일 timestamp 다시 쓸 수 있도록 _last_timestamp 업데이트.
        # "Timestamps must be in chronological order" 에러 방지
        if self._interval is not None:
            self._last_timestamp = self._start_timestamp + self._interval * (self._size - 1)
