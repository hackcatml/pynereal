    ...
    with OHLCVReader(data) as reader:
        ...
        with Progress(
            ...
            try:
                ...
                # bb1d_lower = get_bb1d_lower(data, period=20, mult=2.0, lookahead_on=True)
                # macro_high, macro_low = get_weekly_high_low(data, ago=2, session_offset_hours=9, lookahead_on=True)
                # ################################### Module calculation ####################################
                ...
                with open(config_dir / "realtime_trade.toml", "rb") as f:
                    ...
                    if pyne_section.get("no_report", False):
                        plot_path, strat_path, trade_path = None, None, None
        ...
        if not pyne_section.get("no_progress", False):
